...
export default function CashbackCalculatorPage() {
  const defaults = { spend: 30000, rate: 5, cap: 1000, excluded: 20, fee: 1000, gst: 18 }
  ...
  const r = useMemo(() => {
    ...
    const annualFeeWithGst = fee * (1 + gst / 100)
    ...
    return { eligibleMonthly, uncappedMonthly, monthlyCashback, annualSpend, annualEligible, annualGross, annualFeeWithGst, annualNet, effectiveRate, grossRate, capUtilization, spendToHitCap, breakEvenMonthly }
  }, [spend, rate, cap, excluded, fee, gst])

  const reset = () => { setSpend(defaults.spend); setRate(defaults.rate); setCap(defaults.cap); setExcluded(defaults.excluded); setFee(defaults.fee); setGst(defaults.gst) }
    ...
    <main className="calcPage">
      ...
      <section className="calcShell wrap">
        <div className="calcPanel">
          ...
          <div className="calcNote"><Info size={17}/><p>This calculator assumes the same spending pattern every month. It does not include milestone rewards, joining benefits, accelerated merchant-specific rates or fee-waiver thresholds.</p></div>
        </div>
        ...
        <aside className="resultPanel">
          ...
          <div className="resultGrid">
            ...
            <div><small>Fee incl. GST</small><b>{inr.format(r.annualFeeWithGst)}</b></div>
          </div>
