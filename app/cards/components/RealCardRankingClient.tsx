...
import { Check, ExternalLink, Filter, Info, RotateCcw, Share2, ShieldCheck, Trophy } from 'lucide-react'
import { cardCategoryMap, type CardCategorySlug } from '../../data/card-categories'
import { REAL_CARD_DATA_DATE, type RealCardTerms, type VerifiedRealCard } from '../../data/verified-real-cards'
...
const PROFILE_STORAGE_KEY = 'credonomics-card-profile-v1'

export default function RealCardRankingClient({ categorySlug, cards }: { categorySlug: CardCategorySlug; cards: VerifiedRealCard[] }) {
  ...
  useEffect(() => {
    try {
      ...
      const hasSharedProfile = ['spend','annual','foreign','lounge','loungeValue','partner','network'].some((key) => params.has(key))

      if (hasSharedProfile) {
        setProfile((current) => ({
          ...current,
          monthlySpend: Math.max(0, Number(params.get('spend') ?? current.monthlySpend) || 0),
          annualRetailSpend: Math.max(0, Number(params.get('annual') ?? current.annualRetailSpend) || 0),
          annualForeignSpend: Math.max(0, Number(params.get('foreign') ?? current.annualForeignSpend) || 0),
          loungeVisits: Math.max(0, Number(params.get('lounge') ?? current.loungeVisits) || 0),
          loungeValue: Math.max(0, Number(params.get('loungeValue') ?? current.loungeValue) || 0),
          partnerSpendShare: Math.min(100, Math.max(0, Number(params.get('partner') ?? current.partnerSpendShare) || 0)),
          preferredFuelNetwork: params.get('network') || current.preferredFuelNetwork,
        }))
        if (params.has('otherNetworks')) setShowAllNetworks(params.get('otherNetworks') !== '0')
      } else {
  ...
  const shareRanking = async () => {
    try {
      ...
      url.searchParams.set('spend', String(profile.monthlySpend))
      url.searchParams.set('annual', String(profile.annualRetailSpend))
      url.searchParams.set('foreign', String(profile.annualForeignSpend))
      url.searchParams.set('lounge', String(profile.loungeVisits))
      url.searchParams.set('loungeValue', String(profile.loungeValue))
      url.searchParams.set('partner', String(profile.partnerSpendShare))
      url.searchParams.set('network', profile.preferredFuelNetwork)
      url.searchParams.set('otherNetworks', showAllNetworks ? '1' : '0')
        ...
        <aside className={styles.profile}>
          <div className={styles.profileTop}>
            ...
            <div className={styles.profileActions}>
              <button type="button" onClick={shareRanking}>{copied ? <Check size={14}/> : <Share2 size={14}/>} {copied ? 'Copied' : 'Share'}</button>
          ...
          <div className={styles.profileSaved}>Profile auto-saved on this device · Share creates a reusable ranking link</div>
                ...
                <article className={`${styles.rankCard} ${index === 0 ? styles.winner : ''}`} key={result.card.id}>
                  <div className={styles.cardHead}>
                    ...
                    <div className={styles.identity}>
                      ...
                      <h4><a href={`/cards/${result.card.id}`}>{result.card.name}</a></h4>
                  ...
                  <div className={styles.source}>
                    ...
                    <div className={styles.sourceLinks}>
                      <a href={`/cards/${result.card.id}`}>Full card research</a>
