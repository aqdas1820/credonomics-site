...
SCHEME_PRODUCT_RE = re.compile(r"\b(fund|etf|fof|fund of funds?)\b", re.I)
SCHEME_SUFFIX_RE = re.compile(r"\b(fund|etf|fof|fund of funds?)\b[^\w]{0,6}$", re.I)
TRAILING_SYMBOLS_RE = re.compile(r"[\s,;:|•·†‡*#~^]+$")

DESCRIPTIVE_SUFFIX_PATTERNS = [
    r"\s*\(\s*an?\s+open[- ]ended\s+exchange\s+traded\s+fund\s*\)\s*$",
    r"\s*\(\s*open[- ]ended\s+exchange\s+traded\s+fund\s*\)\s*$",
    r"\s*[-–—]\s*an?\s+open[- ]ended\s+exchange\s+traded\s+fund\s*$",
    r"\s*[-–—]\s*open[- ]ended\s+exchange\s+traded\s+fund\s*$",
    r"\s*\(\s*an?\s+open[- ]ended\s+scheme.*?\)\s*$",
    r"\s*\(\s*open[- ]ended\s+scheme.*?\)\s*$",
]

KNOWN_ALIASES = {
    # canonical key -> final site name
    "hdfc sensex etf": "HDFC Sensex ETF",
    "hdfc nifty etf": "HDFC Nifty ETF",
}
...
def smart_title_token(token: str) -> str:
    ...
    if "-" in token:
        return "-".join(smart_title_token(x) for x in token.split("-"))

    if token.isupper() and len(token) <= 5:
        return token

    if low in {"and", "of", "the", "for", "to"}:
        return low

    return token[:1].upper() + token[1:].lower() if token else token


def strip_descriptive_suffixes(text: str) -> str:
    out = text
    changed = True
    while changed:
        changed = False
        for pattern in DESCRIPTIVE_SUFFIX_PATTERNS:
            newer = re.sub(pattern, "", out, flags=re.I)
            if newer != out:
                out = newer.strip()
                changed = True
    return out
...
def canonicalize_scheme_name(value: str) -> str:
    ...
    # Remove obvious footnote/decorative tails first.
    text = TRAILING_SYMBOLS_RE.sub("", text)
    text = re.sub(r"^[\s,;:|•·†‡*#~^]+", "", text)

    # Remove long scheme-description suffixes.
    text = strip_descriptive_suffixes(text)

    # Remove plan/option tails from base identity.
    ...
    # Normalize common cap-style variants.
    ...
    # Title-normalize while preserving important acronyms.
    tokens = text.split()
    text = " ".join(smart_title_token(tok) for tok in tokens)
    ...
    text = TRAILING_SYMBOLS_RE.sub("", text).strip()

    alias_key = re.sub(r"\s+", " ", text).casefold()
    if alias_key in KNOWN_ALIASES:
        return KNOWN_ALIASES[alias_key]
...
def detect_page_scheme_headings(lines: list[list[dict]]) -> list[tuple[int, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        for segment in split_merged_scheme_line(raw):
            canonical = canonicalize_scheme_name(segment)
            if is_strong_scheme_heading(canonical, avg_size, page_median):
                headings.append((i, canonical))
        ...
        if i + 1 < len(lines):
            ...
            for segment in split_merged_scheme_line(combined):
                canonical = canonicalize_scheme_name(segment)
                if canonical.lower().startswith("hdfc ") and is_strong_scheme_heading(
                    canonical, combined_size, page_median
                ):
                    headings.append((i, canonical))

    # De-duplicate canonical names that occur next to each other.
    ...
    for idx, name in headings:
        ...
        deduped.append((idx, canonicalize_scheme_name(name)))
...
def scheme_for_holding_line(
    ...
    if eligible:
        return canonicalize_scheme_name(eligible[-1])
...
def parse_isin_line(line: list[dict], page_number: int, asset_class: str) -> dict | None:
    ...
    candidates = [
        x for x in re.findall(r"IN[A-Z0-9]{10}", compact)
        if valid_indian_isin(x)
    ]
    ...
    return {
        ...
        "Extraction_Method": "Word-Anchored ISIN V4.5",
...
def parse_pdf(pdf_path: Path, inventory_scheme: str, debug_schemes: bool = False) -> list[dict]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            page_compact = re.sub(
                r"\s+", "", "\n".join(line_text(x) for x in lines)
            ).upper()
    ...
    if debug_schemes:
        ...
        for name in detected_names:
            canonical = canonicalize_scheme_name(name)
...
def main() -> int:
    ...
    date_col = choose_column(
        inventory, ["Report_Date", "Report Date", "Date", "Month"]
    )
    ...
    if args.amc and amc_col:
        inventory = inventory[
            inventory[amc_col].astype(str).str.contains(
                args.amc, case=False, na=False
            )
        ]
    ...
    for _, item in inventory.iterrows():
        ...
        try:
            ...
            print(
                f"[OK] {pdf_path.name}: "
                f"{len(found)} rows | {scheme_count} schemes"
            )
    ...
    if not holdings.empty:
        ...
        holdings = holdings[
            holdings["ISIN"].map(valid_indian_isin)
        ].copy()
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if not holdings.empty:
            ...
            scheme_inventory.to_excel(
                writer,
                sheet_name="Scheme Inventory",
                index=False,
            )
    ...
    print()
    print("=" * 78)
    print("CREDONOMICS MF EXTRACTOR V4.5 COMPLETE")
    print("=" * 78)
    print(f"Holdings rows : {len(holdings):,}")
    print(
        f"Unique ISINs  : "
        f"{holdings['ISIN'].nunique() if not holdings.empty else 0:,}"
    )
    print(
        f"AMCs          : "
