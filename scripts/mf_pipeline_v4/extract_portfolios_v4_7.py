...
def _company_shape_ok(company):
    ...
    if len(c)<3 or len(c)>115 or len(c.split())>14: return False
    ...
    if re.search(r"\b(last|year|month|inception|average|yield|ratio|option|units?)\b",low): return False
    # Require either a recognizable corporate suffix or at least two name-like words.
...
def split_row_from_visual_line(line,page_width,region):
    ...
    wi,weight,wx=w
    if abs(wx-region["weight_x"])>50: return None
    ...
    # Industry must itself look textual, preventing performance/date rows.
    if len(industry)<3 or not re.search(r"[A-Za-z]",industry): return None

    return {
        "Company":company,
        "Industry":industry,
        "Portfolio_Weight_Percent":weight,
        "Extraction_Method":"Column-Aware Coordinate V4.7"
    }
...
def main():
    ...
    print("\n"+"="*78)
    print("CREDONOMICS MF EXTRACTOR V4.7 COMPLETE")
    print("="*78)
