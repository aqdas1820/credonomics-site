...
def infer_portfolio_region(lines,page_width):
    """
    V4.9:
    Detect a dense portfolio block and learn three x-zones:
      company | industry | weight

    V4.8 already found a reliable weight column. V4.9 also estimates the
    company/industry split from repeated rows, so text from adjacent columns
    is not concatenated into the company name.
    """
    candidates=[]
    ...
    for idx,line in enumerate(lines):
        ...
        # Approximate industry start from the last N tokens matching the hint.
        itoks=industry.split()
        if len(left_words) < len(itoks)+1:
            continue

        industry_start_word = left_words[-len(itoks)]
        ix=float(industry_start_word["x0"])

        candidates.append({
            "idx":idx,
            "weight_x":wx,
            "industry_x":ix,
            "company_x0":float(left_words[0]["x0"]),
        })
    ...
    for i in indices:
        block=[c for c in candidates if i-2 <= c["idx"] <= i+45]
    ...
    wxs=sorted(c["weight_x"] for c in best)
    ixs=sorted(c["industry_x"] for c in best)

    median_wx=wxs[len(wxs)//2]
    median_ix=ixs[len(ixs)//2]

    aligned_weight=sum(1 for x in wxs if abs(x-median_wx)<=35)
    aligned_industry=sum(1 for x in ixs if abs(x-median_ix)<=45)

    if aligned_weight < max(5,int(len(best)*0.70)):
        return None
    if aligned_industry < max(5,int(len(best)*0.55)):
        return None

    # Company occupies the area left of industry; allow slight overlap because
    # PDF glyph coordinates are imperfect.
    company_max_x=max(40.0, median_ix-6)

    return {
        "start":max(0,min(c["idx"] for c in best)-2),
        "end":min(len(lines)-1,max(c["idx"] for c in best)+3),
        "weight_x":median_wx,
        "industry_x":median_ix,
        "company_max_x":company_max_x,
    }

def split_row_from_visual_line(line,page_width,region):
    """
    V4.9 row reconstruction.

    Instead of joining all words left of the percentage and then guessing the
    split, reconstruct Company and Industry from learned x-zones.

    This directly targets V4.8 failures such as:
      "Infosys Ltd. Software 8.77" leaking into Company
      while the next column became the Industry.
    """
    txt=line_text(line)
    ...
    industry_x=region["industry_x"]

    # Primary x-zone reconstruction.
    company_words=[
        x for x in words
        if float(x["x0"]) < industry_x-3
    ]
    industry_words=[
        x for x in words
        if industry_x-12 <= float(x["x0"]) < wx-4
    ]

    company=clean_text(" ".join(str(x.get("text","")) for x in company_words))
    industry=clean_text(" ".join(str(x.get("text","")) for x in industry_words))

    # PDF rows sometimes start industry slightly left/right of the learned
    # boundary. If the direct industry zone is not recognized, use the
    # trailing-industry matcher only on the text before the weight.
    if industry.lower() not in {x.lower() for x in INDUSTRY_HINTS}:
        left=clean_text(" ".join(str(x.get("text","")) for x in words))
        fallback_company,fallback_industry=_split_company_industry(left)

        if fallback_industry.lower() not in {x.lower() for x in INDUSTRY_HINTS}:
            return None
    ...
    # Strong column geometry.
    if company_words:
        company_x0=min(float(x["x0"]) for x in company_words)
        company_x1=max(float(x["x1"]) for x in company_words)
        if company_x0 > page_width*0.50:
            return None
        if company_x1 > industry_x+18:
            return None
    ...
    # Reject obvious broken rows where a percentage/value was absorbed into
    # company text before the final weight column.
    numeric_inside_company=re.findall(r"\b\d+(?:\.\d+)?\b",company)
    if len(numeric_inside_company)>=2:
        return None
    ...
    return {
        ...
        "Extraction_Method":"Column-Reconstructed Coordinate V4.9"
    }
...
def main():
    ...
    print("\n"+"="*78)
    print("CREDONOMICS MF EXTRACTOR V4.9 COMPLETE")
    print("="*78)
