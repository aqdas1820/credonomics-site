...
def is_strong_scheme_heading(text: str, avg_size: float, page_median_size: float) -> bool:
    """
    Conservative rule:
    - Must start with HDFC.
    - Must contain Fund / ETF / FOF as a product name.
    - Must not look like a data row, benchmark label or plan/option line.
    - Prefer headings larger than body text, but allow normal-size lines if
      the name structure is very strong.
    """
    text = clean_scheme_name(text)
    low = text.lower()
    ...
    if "IN" in text.upper() and re.search(r"IN[A-Z0-9]{10}", text.upper()):
        return False
    ...
    if not any(
        re.search(rf"\b{re.escape(ending)}\b", low)
        for ending in SCHEME_ENDINGS
    ):
        return False

    # Text should end reasonably close to a scheme product word.
    if not re.search(r"\b(fund|etf|fof|fund of funds?)\b[^\w]{0,4}$", low):
        return False

    # Strong typography OR very clean scheme-shaped line.
...
def detect_page_scheme_headings(lines: list[list[dict]]) -> list[tuple[int, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        if is_strong_scheme_heading(text, avg_size, page_median):
            headings.append((i, clean_scheme_name(text)))
            continue

        # Two-line heading support:
        # line 1 begins "HDFC ..." and line 2 finishes with Fund / ETF / FOF.
        if i + 1 < len(lines):
            t1 = clean_text(text)
            t2 = clean_text(line_text(lines[i + 1]))
            combined = clean_scheme_name(f"{t1} {t2}")
            combined_size = max(avg_size, line_font_size(lines[i + 1]))

            if (
                t1.lower().startswith("hdfc ")
                and len(t1) <= 70
                and is_strong_scheme_heading(combined, combined_size, page_median)
            ):
                headings.append((i, combined))

    # De-duplicate adjacent repeated headings.
    deduped: list[tuple[int, str]] = []
    for idx, name in headings:
        if deduped and name == deduped[-1][1] and idx - deduped[-1][0] <= 3:
            continue
...
def scheme_for_holding_line(
    ...
    if eligible:
        return eligible[-1]
    if carried_scheme:
        return carried_scheme
    inv = clean_scheme_name(inventory_scheme)
...
def parse_isin_line(line: list[dict], page_number: int, asset_class: str) -> dict | None:
    ...
    return {
        ...
        "Extraction_Method": "Word-Anchored ISIN V4.3",
...
def parse_pdf(pdf_path: Path, inventory_scheme: str, debug_schemes: bool = False) -> list[dict]:
    holdings: list[dict] = []
    carried_scheme = clean_scheme_name(inventory_scheme)
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            try:
                ...
                # fallback for pdfplumber builds that do not expose font attrs
                try:
                    words = page.extract_words(
                        x_tolerance=2,
                        y_tolerance=2,
            ...
            if "IN" not in page_compact:
                # Still collect a genuine heading to carry into the next page.
                headings = detect_page_scheme_headings(lines)
                if headings:
                    carried_scheme = headings[-1][1]
                    detected_names.extend([x[1] for x in headings])
                continue
    ...
    if debug_schemes:
        ...
        for name in detected_names:
            if name not in unique:
                unique.append(name)
        print("    detected scheme headings:")
        for name in unique:
            print(f"      - {name}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug-schemes",
        action="store_true",
        help="Print every distinct scheme heading detected in each PDF.",
    )
    ...
    print()
    print("=" * 78)
    print("CREDONOMICS MF EXTRACTOR V4.3 COMPLETE")
    print("=" * 78)
