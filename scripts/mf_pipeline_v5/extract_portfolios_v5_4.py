...
def parse_block_row(
    ...
    return {
        ...
        "Extraction_Method": "Logical-Portfolio Coordinate V5.4",
...
def detect_variant_markers(lines: list[list[dict]]) -> list[dict]:
    """Detect explicit plan / variant labels near portfolio blocks."""
    markers = []
    explicit_patterns = [
        r"\bsavings\s+plan\b", r"\binvestment\s+plan\b",
        r"\bwholesale\s+plan\b", r"\bretail\s+plan\b",
        r"\bregular\s+plan\b", r"\bdirect\s+plan\b",
        r"\bplan\s+[A-Z0-9]+\b", r"\bplan[- ]?[A-Z0-9]+\b",
        r"\bgrowth\s+plan\b", r"\bdividend\s+plan\b",
    ]
...
def assign_variant_to_block(lines, block, model, variant_markers):
    ...
    label = clean_text(best["label"])
    if len(label) > 80:
        label = clean_text(best["matched"])
    return label or "UNLABELED_VARIANT", best_score, "local-variant"
...
def canonical_variant_label(label: str) -> str:
    """Normalize noisy historical plan text to a compact variant identity."""
    s = clean_text(label)
    low = s.lower()
    if not s or low == "unlabeled_variant":
    ...
    for pat, name in canonical:
        if re.search(pat, s, flags=re.I):
            return name

    # Preserve explicit numbered/lettered plans but reject prose such as "plan in".
    m = re.search(r"\bplan[- ]+([A-Z0-9]{1,8})\b", s, flags=re.I)
    if m:
        token = m.group(1)
        stop = {"IN", "IS", "OF", "TO", "THE", "AND", "FOR", "ON", "AT", "AS"}
        if token.upper() not in stop:
            return f"Plan {token.upper()}"
...
def canonical_section_label(label: str) -> str:
    s = clean_text(label)
    low = s.lower()
    if not s or low == "unlabeled_section":
    ...
    # Avoid carrying long noisy headings into identity keys.
    return s[:70]
...
def assemble_logical_portfolios(holdings: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    Conservative continuation rule:
      ...
      - compatible clean variant (same explicit variant, or both unlabeled)
      - each physical block is partial (<90%)
      - combined total is 85-115%
      - company overlap <=15% (continuations should add holdings, not duplicate them)
    """
    if holdings.empty:
    ...
    phys = (
        d.groupby([
            ...
            "Clean_Section", "Portfolio_ID", "Physical_Portfolio_ID",
...
def main() -> int:
    ...
    for _, item in inventory.iterrows():
        ...
        try:
            ...
            for rec in rows:
                ...
                rec["Portfolio_ID"] = stable_portfolio_id(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if not holdings.empty:
            ...
            # Conservative: explicit variant + same section + adjacent pages + partial totals.
            for i in range(len(portfolio_audit)):
                a = portfolio_audit.iloc[i]
                if a["Variant_Label"] == "UNLABELED_VARIANT":
                    continue
                ...
                for j in range(i + 1, len(portfolio_audit)):
                    ...
                    if b["Scheme"] != a["Scheme"]:
                        continue
                    if b["Variant_Label"] != a["Variant_Label"]:
                        continue
                    if b["Section_Label"] != a["Section_Label"]:
                        continue
                    if abs(int(b["Page"]) - int(a["Page"])) != 1:
                        continue
    ...
    print()
    print("=" * 82)
    print("CREDONOMICS MF EXTRACTOR V5.4 COMPLETE")
    print("=" * 82)
