...
def parse_block_row(
    ...
    return {
        ...
        "Extraction_Method": "Modern-Scheme-Resolved Coordinate V5.8",
...
def normalize_modern_scheme_heading(text: str) -> str:
    """
    Return a conservative HDFC scheme heading, or "".

    Used only by the 2025+ identity resolver. A valid promoted scheme must:
      - begin with HDFC
      - look like a named scheme, not a generic section
      - terminate as Fund / Index Fund / ETF / FoF

    We also trim a few obvious trailing PDF artifacts after a valid terminal.
    """
    s = clean_text(text)
    if not s:
        return ""

    # Normalize separators but keep scheme wording.
    ...
    # Reject generic headings even if "HDFC" somehow leaked into them.
    if any(x in low for x in [
        "category of scheme",
        "fund manager",
        "www.hdfcfund.com",
    ]) and not re.search(r"\b(?:fund|etf|fof)\b", low):
        return ""

    # Strip obvious trailing website/bullet contamination after a scheme terminal.
    # Examples observed in modern PDFs can include:
    #   HDFC NIFTY Midcap 150 ETF • Hero MotoCorp Ltd. www.hdfcfund.com
    terminal = re.search(
        r"^(HDFC\s+.+?\b(?:Index\s+Fund|ETF|FoF|Fund))"
        r"(?=\s*(?:[•|]|www\.|$))",
        s,
        flags=re.I,
    )
    if terminal:
        candidate = clean_text(terminal.group(1)).strip(" -–—|,;:")
    else:
        # Promotion from Clean_Section should normally already be compact.
        if not re.search(r"\b(?:Index\s+Fund|ETF|FoF|Fund)$", s, flags=re.I):
            return ""
        candidate = s
    ...
    generic_exact = {
        "hdfc mutual fund",
        "hdfc fund",
        "hdfc index fund",  # historical generic family; don't create a modern anchor
    }
    if clow in generic_exact:
        return ""

    if len(candidate.split()) < 3 or len(candidate) > 105:
        return ""

    # Reject obvious prose.
    if re.search(
        r"\b(fund manager|benchmark|returns?|riskometer|expense ratio|"
        r"exit load|yield|maturity|portfolio turnover|category of scheme)\b",
        clow,
    ):
        return ""
...
def modern_target_is_portfolio_like(section: str, variant: str) -> bool:
    """
    Limit same-page/forward recovery to blocks that look like portfolio sections.
    """
    sec = clean_text(section) or "UNLABELED_SECTION"
    var = clean_text(variant) or "UNLABELED_VARIANT"

    if sec in MODERN_GENERIC_SECTIONS:
        return True
    if var in MODERN_GENERIC_VARIANTS:
        return True

    # Named sections that are not another HDFC scheme are allowed.
    if not normalize_modern_scheme_heading(sec) and len(sec.split()) <= 8:
        return True

    return False


def resolve_modern_hdfc_schemes(
    holdings: pd.DataFrame,
    modern_start_year: int = 2025,
    max_forward_pages: int = 2,
) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """
    V5.8 modern HDFC scheme resolver.

    This layer does NOT touch Company / Industry / Weight extraction.

    Recovery order:
      A. SECTION_SCHEME_PROMOTION
         UNASSIGNED + Clean_Section="HDFC ... Fund/ETF/FoF"
         -> promote the section heading to Scheme.

      B. SAME_PAGE_UNIQUE_ANCHOR
         If a page has exactly one distinct strong assigned scheme, remaining
         portfolio-like UNASSIGNED blocks on that page inherit it.

      C. FORWARD_ANCHOR_1P / FORWARD_ANCHOR_2P
         For 2025+ only, look back at most 1-2 physical pages for the nearest
         page containing exactly one distinct STRONG scheme anchor. Propagate
         only if the target page has no strong scheme anchor of its own.
         A new anchor immediately breaks propagation.

    "Strong" anchors are:
      - schemes already assigned before V5.8
      - SECTION_SCHEME_PROMOTION
      - the pre-existing V5.6 SAME_PAGE_EXPLICIT_VARIANT recovery

    SAME_PAGE_UNIQUE_ANCHOR and FORWARD_ANCHOR results never become new strong
    anchors, so recovery cannot chain indefinitely.
    """
    if holdings.empty:
        return holdings.copy(), pd.DataFrame(), {}
    ...
    # Preserve scheme state entering V5.8.
    d["Modern_Scheme_Original"] = d["Scheme"]
    d["Modern_Recovery_Rule"] = "NONE"
    d["Modern_Anchor_Scheme"] = ""
    d["Modern_Anchor_Page"] = pd.Series(
        pd.NA, index=d.index, dtype="Int64"
    )
    d["Modern_Page_Distance"] = pd.Series(
        pd.NA, index=d.index, dtype="Int64"
    )
    d["Modern_Recovery_Confidence"] = ""
    d["Modern_Recovery_Evidence"] = ""

    # Only modern HDFC history is touched.
    years = pd.to_datetime(d["Report_Date"], errors="coerce").dt.year
    modern_mask = (
        years.ge(modern_start_year)
        & d["AMC"].astype(str).str.contains("HDFC", case=False, na=False)
    )

    if not modern_mask.any():
        stats = {
            "existing_assigned": 0,
            "section_promoted": 0,
            "same_page_recovered": 0,
            "forward_1p_recovered": 0,
            "forward_2p_recovered": 0,
            "still_unassigned": 0,
        }
        return d, pd.DataFrame(), stats

    def portfolio_meta(frame: pd.DataFrame) -> pd.DataFrame:
        return (
            frame.groupby(
                [
                    "AMC", "Report_Date", "PDF_File", "Page_Number",
                    "Block_ID", "Portfolio_ID", "Scheme",
                    "Clean_Variant", "Clean_Section",
                    "Scheme_Recovery",
                ],
                dropna=False,
            )
    ...
    # ------------------------------
    # A. Promote scheme-like sections
    # ------------------------------
    meta = portfolio_meta(d.loc[modern_mask].copy())

    for _, r in meta[meta["Scheme"] == "UNASSIGNED"].iterrows():
        promoted = normalize_modern_scheme_heading(r["Clean_Section"])
        if not promoted:
            continue

        mask = d["Portfolio_ID"] == r["Portfolio_ID"]
        d.loc[mask, "Scheme"] = promoted
        d.loc[mask, "Clean_Section"] = "UNLABELED_SECTION"
        d.loc[mask, "Scheme_Source"] = "modern-section-scheme-promotion"
        d.loc[mask, "Modern_Recovery_Rule"] = "SECTION_SCHEME_PROMOTION"
        d.loc[mask, "Modern_Anchor_Scheme"] = promoted
        d.loc[mask, "Modern_Anchor_Page"] = int(r["Page_Number"])
        d.loc[mask, "Modern_Page_Distance"] = 0
        d.loc[mask, "Modern_Recovery_Confidence"] = "High"
        d.loc[mask, "Modern_Recovery_Evidence"] = (
            f"promoted Clean_Section={r['Clean_Section']}"
        )

    # ------------------------------
    # Strong-anchor snapshot
    # ------------------------------
    meta = portfolio_meta(d.loc[modern_mask].copy())

    # A portfolio is a strong anchor if assigned before V5.8, promoted from an
    # explicit section heading, or recovered by V5.6's exact explicit variant.
    strong_portfolio_ids = set()

    for _, r in meta[meta["Scheme"] != "UNASSIGNED"].iterrows():
        pid = r["Portfolio_ID"]
        rows = d[d["Portfolio_ID"] == pid]

        modern_rule = clean_text(rows["Modern_Recovery_Rule"].iloc[0])
        original = clean_text(rows["Modern_Scheme_Original"].iloc[0])
        old_recovery = clean_text(rows["Scheme_Recovery"].iloc[0])

        if (
            original != "UNASSIGNED"
            or modern_rule == "SECTION_SCHEME_PROMOTION"
            or old_recovery == "SAME_PAGE_EXPLICIT_VARIANT"
        ):
            strong_portfolio_ids.add(pid)

    def page_strong_schemes(meta_frame: pd.DataFrame) -> dict:
        result = {}
        sm = meta_frame[meta_frame["Portfolio_ID"].isin(strong_portfolio_ids)]
        for key, g in sm.groupby(
            ["AMC", "Report_Date", "PDF_File", "Page_Number"],
            dropna=False,
        ):
            schemes = sorted({
                clean_text(x)
                for x in g["Scheme"].tolist()
                if clean_text(x) and clean_text(x) != "UNASSIGNED"
            })
            result[key] = schemes
        return result

    strong_by_page = page_strong_schemes(meta)

    # ------------------------------
    # B. Same-page unique anchor
    # ------------------------------
    meta = portfolio_meta(d.loc[modern_mask].copy())

    for _, target in meta[meta["Scheme"] == "UNASSIGNED"].iterrows():
        if not modern_target_is_portfolio_like(
            target["Clean_Section"], target["Clean_Variant"]
        ):
            continue

        key = (
            target["AMC"], target["Report_Date"], target["PDF_File"],
            target["Page_Number"],
        )
        schemes = strong_by_page.get(key, [])
        ...
        scheme = schemes[0]
        mask = d["Portfolio_ID"] == target["Portfolio_ID"]
        d.loc[mask, "Scheme"] = scheme
        d.loc[mask, "Scheme_Source"] = "modern-same-page-unique-anchor"
        d.loc[mask, "Modern_Recovery_Rule"] = "SAME_PAGE_UNIQUE_ANCHOR"
        d.loc[mask, "Modern_Anchor_Scheme"] = scheme
        d.loc[mask, "Modern_Anchor_Page"] = int(target["Page_Number"])
        d.loc[mask, "Modern_Page_Distance"] = 0
        d.loc[mask, "Modern_Recovery_Confidence"] = "High"
        d.loc[mask, "Modern_Recovery_Evidence"] = (
            f"unique strong scheme on page {int(target['Page_Number'])}: {scheme}"
        )

    # IMPORTANT: same-page recovered blocks are NOT added to strong anchors.
    # This prevents them from extending the forward propagation horizon.

    # ------------------------------
    # C. Short forward propagation
    # ------------------------------
    meta = portfolio_meta(d.loc[modern_mask].copy())

    # Strong page anchors still come from the frozen snapshot above.
    # For each target page, nearest previous strong page within 2 pages wins.
    for _, target in meta[meta["Scheme"] == "UNASSIGNED"].iterrows():
        if not modern_target_is_portfolio_like(
            target["Clean_Section"], target["Clean_Variant"]
        ):
        ...
        # Do not forward-fill a page that already has a strong anchor.
        target_key = (amc, report_date, pdf_file, page)
        if strong_by_page.get(target_key):
            continue

        chosen = None

        for distance in range(1, max_forward_pages + 1):
            anchor_page = page - distance
            if anchor_page < 1:
                break

            anchor_key = (amc, report_date, pdf_file, anchor_page)
            schemes = strong_by_page.get(anchor_key, [])

            # The nearest encountered strong page is a boundary. If ambiguous,
            # stop; never jump over it to an older anchor.
            if schemes:
                if len(schemes) == 1:
                    chosen = (schemes[0], anchor_page, distance)
                break

        if not chosen:
            continue

        scheme, anchor_page, distance = chosen
        rule = f"FORWARD_ANCHOR_{distance}P"

        mask = d["Portfolio_ID"] == target["Portfolio_ID"]
        d.loc[mask, "Scheme"] = scheme
        d.loc[mask, "Scheme_Source"] = f"modern-forward-anchor-{distance}p"
        d.loc[mask, "Modern_Recovery_Rule"] = rule
        d.loc[mask, "Modern_Anchor_Scheme"] = scheme
        d.loc[mask, "Modern_Anchor_Page"] = anchor_page
        d.loc[mask, "Modern_Page_Distance"] = distance
        d.loc[mask, "Modern_Recovery_Confidence"] = (
            "High" if distance == 1 else "Medium"
        )
        d.loc[mask, "Modern_Recovery_Evidence"] = (
            f"nearest strong anchor page={anchor_page}; "
            f"distance={distance}; scheme={scheme}"
        )
    ...
    def modern_identity(r):
        rule = clean_text(r.get("Modern_Recovery_Rule", ""))
        if rule == "SECTION_SCHEME_PROMOTION":
            return "Recovered — Modern Section Scheme"
        if rule == "SAME_PAGE_UNIQUE_ANCHOR":
            return "Recovered — Modern Same Page"
        if rule.startswith("FORWARD_ANCHOR_"):
            return "Recovered — Modern Forward Anchor"
    ...
    d["Identity_Status"] = d.apply(modern_identity, axis=1)

    # ------------------------------
    # Audit: one row per modern physical portfolio
    # ------------------------------
    final_meta = (
        d.loc[modern_mask]
        .groupby(
            [
                "AMC", "Report_Date", "PDF_File", "Page_Number", "Block_ID",
                "Portfolio_ID", "Modern_Scheme_Original", "Scheme",
                "Clean_Variant", "Clean_Section", "Modern_Recovery_Rule",
                "Modern_Anchor_Scheme", "Modern_Anchor_Page",
                "Modern_Page_Distance", "Modern_Recovery_Confidence",
                "Modern_Recovery_Evidence",
            ],
            dropna=False,
        )
        .agg(
        ...
        .reset_index()
        .sort_values(
            ["Report_Date", "PDF_File", "Page_Number", "Block_ID"]
        )
    )

    final_meta["Modern_Status"] = final_meta.apply(
        lambda r: (
            "STILL_UNASSIGNED"
            if r["Scheme"] == "UNASSIGNED"
            else (
                "EXISTING_ASSIGNED"
                if r["Modern_Recovery_Rule"] == "NONE"
                else r["Modern_Recovery_Rule"]
            )
        ),
        axis=1,
    )
    ...
    stats = {
        "existing_assigned": int(
            (final_meta["Modern_Status"] == "EXISTING_ASSIGNED").sum()
        ),
        "section_promoted": int(
            (final_meta["Modern_Status"] == "SECTION_SCHEME_PROMOTION").sum()
        ),
        "same_page_recovered": int(
            (final_meta["Modern_Status"] == "SAME_PAGE_UNIQUE_ANCHOR").sum()
        ),
        "forward_1p_recovered": int(
            (final_meta["Modern_Status"] == "FORWARD_ANCHOR_1P").sum()
        ),
        "forward_2p_recovered": int(
            (final_meta["Modern_Status"] == "FORWARD_ANCHOR_2P").sum()
        ),
        "still_unassigned": int(
            (final_meta["Modern_Status"] == "STILL_UNASSIGNED").sum()
        ),
    }

    return d, final_meta, stats
...
def main() -> int:
    ...
    modern_scheme_audit = pd.DataFrame()
    modern_scheme_stats = {}

    if not holdings.empty:
        holdings, recovery_audit = recover_unassigned_schemes(holdings)
        holdings, modern_scheme_audit, modern_scheme_stats = (
            resolve_modern_hdfc_schemes(holdings)
        )
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if not modern_scheme_audit.empty:
            modern_scheme_audit.to_excel(
                writer, sheet_name="Modern Scheme Audit", index=False
            )
            modern_summary = (
                modern_scheme_audit.groupby(
                    ["Report_Date", "Modern_Status"],
                    dropna=False,
                )
                .agg(Portfolios=("Portfolio_ID", "nunique"))
                .reset_index()
                .sort_values(["Report_Date", "Modern_Status"])
            )
            modern_summary.to_excel(
                writer, sheet_name="Modern Scheme Summary", index=False
            )
        ...
        if not holdings.empty:
            ...
            portfolio_audit["Continuation_Candidate"] = pd.Series(
                "", index=portfolio_audit.index, dtype="string"
            )
            portfolio_audit["Continuation_With"] = pd.Series(
                "", index=portfolio_audit.index, dtype="string"
            )
            portfolio_audit["Continuation_Overlap_Pct"] = pd.Series(
                np.nan, index=portfolio_audit.index, dtype="float64"
            )
            portfolio_audit["Continuation_Decision"] = pd.Series(
                "", index=portfolio_audit.index, dtype="string"
            )
    ...
    manifest["modernSchemeResolver"] = modern_scheme_stats
    (export_dir / "manifest.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    print()
    print("=" * 82)
    print("CREDONOMICS MF EXTRACTOR V5.8 COMPLETE")
    print("=" * 82)
    ...
    if modern_scheme_stats:
        print(
            "Modern scheme : "
            f"existing={modern_scheme_stats.get('existing_assigned', 0)}, "
            f"section={modern_scheme_stats.get('section_promoted', 0)}, "
            f"same-page={modern_scheme_stats.get('same_page_recovered', 0)}, "
            f"forward-1p={modern_scheme_stats.get('forward_1p_recovered', 0)}, "
            f"forward-2p={modern_scheme_stats.get('forward_2p_recovered', 0)}, "
            f"unassigned={modern_scheme_stats.get('still_unassigned', 0)}"
        )
